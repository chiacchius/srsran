...
{
  pdcp_entity_lte(rlc_, rrc_, gw_, task_sched_, logger, lcid_, -1);
}
...
void pdcp_entity_lte::write_sdu(unique_byte_buffer_t sdu, int upper_sn)
{
  ...
  */
  // Uplink message on side of the UE
  printf("\n\033[32mUPLINK\033[0m Message with size: %d\n", sdu->N_bytes);
  enc = 1;
  link = UPLINK;
  ...
  for(size_t i=0; i<sdu->N_bytes; ++i) {
        printf("\033[32m%02x \033[0m", (unsigned char)sdu->msg[i]);
  }
  printf("\n");
 /*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
...
void pdcp_entity_lte::handle_srb_pdu(srsran::unique_byte_buffer_t pdu)
{
   ...
   * Author: Matteo Chiacchia
  */
  // Downlink message on side of the UE
  printf("\n\033[31mDOWNLINK\033[0m Message with size: %d\n", pdu->N_bytes);
  enc = 1;
  link = DOWNLINK;
  char *ue_env_var = getenv("UE_VAR");
  if (ue_env_var != NULL){
  ...
  for(size_t i=0; i<pdu->N_bytes; ++i) {
        printf("\033[31m%02x \033[0m", (unsigned char)pdu->msg[i]);
  }
  printf("\n");
 /*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
